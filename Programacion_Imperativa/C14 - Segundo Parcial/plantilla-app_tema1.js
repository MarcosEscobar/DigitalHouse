...
let appProfesionales = {
  ...
  listarProfesionales : function listarProfesionales() {
    for (let i = 0; i < this.profesionales.length; i++) {
      ...
      estado = (profesional.estaHabilitado == true) ? 'ok' : 'Inactivo';
      console.log('Id ' + profesional.id + ' ' +  estado + ', ' + profesional.especialidad + ', ' + profesional.nombre + ', valor de consulta $' + profesional.honorarioConsulta + ', puntaje: ' + profesional.puntaje);
    }
  },
  ...
  filtrarMejoresPuntajes : function () {
    ...
    for (let i = 0; i < this.profesionales.length; i++) {
      if (this.profesionales[i].puntaje >= 7) {
        aprobados.push(this.profesionales[i]);
      }
    }
    return aprobados;
  },
...
// E
  habilitarProfesional : function (id) {
    this.buscarPorID(id).estaHabilitado = true;
  },
// F
  corregirNombres : function () {
    for (let i = 0; i < this.profesionales.length; i++) {
      this.profesionales[i].nombre =  this.profesionales[i].nombre.charAt(0).toUpperCase() + this.profesionales[i].nombre.slice(1);
    } 
  }
}
...
console.log(appProfesionales.buscarPorID(11));
console.log(appProfesionales.habilitarProfesional(11));
console.log(appProfesionales.buscarPorID(11));
console.log(o);

console.log(v, oo + " F. corregirNombres");
appProfesionales.corregirNombres();
appProfesionales.listarProfesionales();
console.log(o);
